...
int api_queries(struct ftl_conn *api)
{
	...
	unsigned long cursor, largest_db_index, dbnum;
	memdb_size(&largest_db_index, &dbnum);
	...
	if(api->request->query_string != NULL)
	{
		...
		if(GET_VAR("type", buffer, api->request->query_string) > 0)
		{
			...
			add_querystr_string(api, querystr, "type=", ":type");
		}
		...
		if(GET_VAR("status", buffer, api->request->query_string) > 0)
		{
			...
			add_querystr_string(api, querystr, "status=", ":status");
		}
		...
		if(GET_VAR("reply", buffer, api->request->query_string) > 0)
		{
			...
			add_querystr_string(api, querystr, "reply=", ":reply");
		}
		...
		if(GET_VAR("dnssec", buffer, api->request->query_string) > 0)
		{
			...
			add_querystr_string(api, querystr, "dnssec=", ":dnssec");
		}
	}
	...
	if(config.debug & DEBUG_API)
		logg("SQL: %s, cursor: %lu", querystr, cursor);
	...
	while((rc = sqlite3_step(read_stmt)) == SQLITE_ROW)
	{
		...
		if((unsigned long)id > cursor - start)
		{
			// Skip all results with id BEFORE cursor - start
			continue;
		}
		else if(length > 0 && added >= (unsigned int)length)
		{
			// Skip everything AFTER we added the requested number
			// of queries. Length may be set to -1 to indicate we
			// want everything (so we never skip here)
			continue;
		}
	...
	JSON_OBJ_ADD_NUMBER(json, "recordsTotal", dbnum);
