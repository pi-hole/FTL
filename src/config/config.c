...
bool cluster_hashable_ignoring_transport(const struct conf_item *item)
{
	if(item->f & FLAG_CREDENTIAL)
		...
		return !(item->f & (FLAG_API_CLI_READ_ONLY | FLAG_PSEUDO_ITEM));

	return !(item->f & (FLAG_NO_CLUSTER | FLAG_API_CLI_READ_ONLY | FLAG_WRITE_ONLY |
	                    FLAG_PSEUDO_ITEM));
}
...
bool cluster_syncable_ignoring_transport(const struct conf_item *item)
{
	if(item->f & FLAG_CREDENTIAL)
		return config.cluster.sync.credentials.v.b &&
		       !(item->f & (FLAG_ENV_VAR | FLAG_API_CLI_READ_ONLY | FLAG_PSEUDO_ITEM));

	return !(item->f & (FLAG_NO_CLUSTER | FLAG_ENV_VAR | FLAG_API_CLI_READ_ONLY |
...
void initConfig(struct config *conf)
{
	...
	conf->webserver.advancedOpts.f = FLAG_RESTART_FTL | FLAG_API_READ_ONLY | FLAG_NO_CLUSTER;
	...
	conf->webserver.paths.webroot.f = FLAG_RESTART_FTL | FLAG_API_READ_ONLY | FLAG_NO_CLUSTER;
	...
	conf->webserver.paths.webhome.f = FLAG_RESTART_FTL | FLAG_API_READ_ONLY | FLAG_NO_CLUSTER;
	...
	conf->webserver.paths.prefix.f = FLAG_RESTART_FTL | FLAG_API_READ_ONLY | FLAG_NO_CLUSTER;
	...
	conf->misc.dnsmasq_lines.f = FLAG_RESTART_FTL | FLAG_API_READ_ONLY | FLAG_NO_CLUSTER;
...
bool getLogFilePath(bool try_read)
{
	...
	config.files.log.ftl.f = FLAG_RESTART_FTL | FLAG_NO_CLUSTER;
                       ...
                       const bool strict, struct config_apply *result)
{
	...
	for(unsigned int i = 0; i < CONFIG_ELEMENTS; i++)
	{
		...
		// An item that can only be set on the host is not taken from a
		// document either, see FLAG_API_READ_ONLY. Only an actual change
		// gets here, a document naming the current value is fine
		if(item->f & FLAG_API_READ_ONLY)
		{
			result->failed_key = item->k;
			result->failure = CONFIG_APPLY_API_READ_ONLY;
			if(strict)
			{
				free_config(&trial, false);
				return false;
                    ...
                    const bool from_cluster, const double changed, char *errbuf)
{
	...
	// The path rules span several items and are checked on the assembled
	// configuration, before the dnsmasq test installs the file it tests
	char patherr[VALIDATOR_ERRBUF_LEN] = { 0 };
	if(!validate_config_paths(newconf, patherr, NULL))
	{
		snprintf(errbuf, ERRBUF_SIZE, "%s", patherr);
		free_config(newconf, false);
		return false;
	}

	if(result->dnsmasq_changed && !write_dnsmasq_config(newconf, DNSMASQ_TEST_INSTALL, errbuf))
	{
		free_config(newconf, false);
		return false;
	}
