...
*  Logging routines
*
 *  Since the asynchronous logging rewrite (logger.c) this file contains the
 *  producer side: formatting log records and passing them to the shared
 *  logger thread.  All log-file I/O (FTL.log, webserver.log, pihole.log)
 *  happens in the logger thread; the same thread fills the in-memory FIFO
 *  (API /logs) through the add_to_fifo_buffer() helper kept in this file.
*
...
// care of the actual sinks; records dropped because the queue is full are
// silently discarded.  This function never blocks.
static void __attribute__ ((format(printf, 2, 0))) log_record_format_and_push(struct log_record *rec, const char *format, va_list args)
{
...
void __attribute__ ((format (printf, 3, 4))) _FTL_log(const int priority, const enum debug_flag flag, const char *format, ...)
{
	...
	// Print to stdout (human-readable) before enqueueing.  This stays
	// synchronous like before.  The va_list is passed to vprintf() directly
	// and must not be reused afterwards, so the record is formatted from an
	// independent copy taken up front.
	va_list args_store;
	...
	if(print_log)
	{
		...
		log_record_format_and_push(&rec, format, args_store);
	}
...
void __attribute__ ((format (printf, 3, 4))) _log_web(const int priority, const enum debug_flag flag, const char *format, ...)
{
	...
	// Print to stdout (human-readable) before enqueueing.  See _FTL_log()
	// for why the record is formatted from an independent va_list copy.
	va_list args_store;
	va_copy(args_store, args);
