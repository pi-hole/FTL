...
@test "API: Correct answer to summary request (including header check)" {
  ...
  [[ ${lines[5]} == "{\"domains_being_blocked\":-1,\"dns_queries_today\":7,\"ads_blocked_today\":2,\"ads_percentage_today\":28.5714,\"unique_domains\":6,\"queries_forwarded\":3,\"queries_cached\":2,\"clients_ever_seen\":3,\"unique_clients\":3}" ]]
  [[ "${status}" -eq 0 ]]
}
