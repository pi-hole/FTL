...
@test "Config validation working on the CLI (validator-based checking)" {
  ...
  printf "%s\n" "${lines[@]}"
  [[ ${lines[0]} == 'New dnsmasq configuration is not valid ('*'resolve server name at line '*' of /etc/pihole/dnsmasq.conf.temp: "rev-server=1.1.1.1,def"), config remains unchanged' ]]
  [[ $status == 3 ]]
